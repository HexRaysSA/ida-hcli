"""Tests for orchestrate_install and orchestrate_upgrade."""
...
def test_orchestrate_install_success(virtual_ida_environment):
...
def test_orchestrate_install_already_installed_fails(virtual_ida_environment):
    ctx = make_test_install_context()
    buf = (PLUGINS_DIR / "plugin1" / "plugin1-v1.0.0.zip").read_bytes()
    _, metadata = get_metadata_from_plugin_archive(buf, "plugin1")
...
def test_orchestrate_install_with_settings(virtual_ida_environment):
    ctx = make_test_install_context()
...
def test_orchestrate_install_bad_settings_rolls_back(virtual_ida_environment):
    ctx = make_test_install_context()
    buf = (PLUGINS_DIR / "plugin1" / "plugin1-v5.0.0.zip").read_bytes()
    _, metadata = get_metadata_from_plugin_archive(buf, "plugin1")
...
def test_orchestrate_install_no_repo_warns_about_deps(virtual_ida_environment):
...
def test_orchestrate_install_never_raises(virtual_ida_environment):
...
def test_orchestrate_upgrade_success(virtual_ida_environment):
    ctx = make_test_install_context()
    ...
    r1 = orchestrate_install(source=v1, plugin_name="plugin1", metadata=meta1, ctx=ctx)
    assert r1.status == InstallStatus.SUCCESS

    r2 = orchestrate_upgrade(zip_data=v2, plugin_name="plugin1", metadata=meta2, ctx=ctx)
...
def test_orchestrate_upgrade_downgrade_fails(virtual_ida_environment):
    ctx = make_test_install_context()
    v1 = (PLUGINS_DIR / "plugin1" / "plugin1-v1.0.0.zip").read_bytes()
    v2 = (PLUGINS_DIR / "plugin1" / "plugin1-v2.0.0.zip").read_bytes()
    _, meta1 = get_metadata_from_plugin_archive(v1, "plugin1")
    ...
    orchestrate_install(source=v2, plugin_name="plugin1", metadata=meta2, ctx=ctx)

    result = orchestrate_upgrade(zip_data=v1, plugin_name="plugin1", metadata=meta1, ctx=ctx)
    assert result.status == InstallStatus.FAILED
    assert result.reason is not None
    assert "not greater" in result.reason.lower() or "downgrade" in result.reason.lower()


def test_orchestrate_upgrade_never_raises(virtual_ida_environment):
    ctx = make_test_install_context()
    v1 = (PLUGINS_DIR / "plugin1" / "plugin1-v1.0.0.zip").read_bytes()
    _, meta1 = get_metadata_from_plugin_archive(v1, "plugin1")

    orchestrate_install(source=v1, plugin_name="plugin1", metadata=meta1, ctx=ctx)
