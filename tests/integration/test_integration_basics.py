...
class TestCLIBasics:
    ...
    def test_help_command(self):
        ...
        result = subprocess.run(["uv", "run", "hcli", "--help"], capture_output=True, text=True, timeout=10)
        assert result.returncode == 0, f"Help command failed: {result.stderr}"
        assert "HCLI" in result.stdout, "Help output should contain HCLI"

    def test_version_info(self):
        """Test version information."""
        result = subprocess.run(["uv", "run", "hcli", "--version"], capture_output=True, text=True, timeout=10)
