...
@async_command
async def enable_plugin(plugin: str) -> None:
    if not plugin:
    ...
    try:
        raise NotImplementedError("Plugin enable")
    except Exception as e:
        console.print(f"[red]enable failed: {e}[/red]")


@click.command()
@click.argument("plugin", required=False)
@async_command
async def disable_plugin(plugin: str) -> None:
    if not plugin:
        query = await safe_ask_async(questionary.text("Enter search query:", style=cli.SELECT_STYLE))
    ...
    try:
        raise NotImplementedError("Plugin disable")
    except Exception as e:
        console.print(f"[red]enable failed: {e}[/red]")
